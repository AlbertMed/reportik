...
class Mod_AlmacenGralController extends Controller
{
    ...
    public function DataShowEntradas(Request $request)
    {
        ...
        $sociedadPDF = 'PIXVS'; // COMERCIALIZADORA PIXVS e ITEKNIA PIXVS
        $razonSocial = 'ITEKNIA EQUIPAMIENTO S.A. de C.V. ';
        if ($request->get('sociedad') == 'COMERCIALIZADORA') {
            $sociedad = 'comercializadora'; //PIXVS
            $razonSocial = 'COMERCIALIZADORA ITEKNIA S.A. de C.V.';
        } else if($request->get('sociedad') == 'MULIIX'){
            $sociedad = 'muliix';
            $sociedadPDF = 'MULIIX';
        } else {
            $sociedad = 'sqlsrv';  // ITEKNIA PIXVS            
        }
        if ($sociedad == 'muliix') {
            $data = DB::connection($sociedad)->select( "
            Select	OC_CodigoOC as ORDEN, OCRC_FechaRecibo AS F_RECIBO, PRO_CodigoProveedor AS CLIENTE, 
            PRO_NombreComercial AS RAZON_SOC, OCRC_Comentarios AS NOTAS, PRY_CodigoEvento AS C_PROY, 
            PRY_NombreProyecto AS PROYECTO, ISNULL(ART_CodigoArticulo, 'MISC') AS CODE_ART,
             OCD_DescripcionArticulo AS ARTICULO, OCD_CMUM_UMCompras AS UMC, 
             CASE WHEN OCD_AFC_FactorConversion > 0 THEN OCD_AFC_FactorConversion  ELSE 1 END AS FACT, 
             (Select CMUM_Nombre from ControlesMaestrosUM Where CMUM_UnidadMedidaId = ISNULL(ART_CMUM_UMInventarioId,'5A929D93-D43D-4F8F-8985-0624A61C5A82')) AS UMI,
              CANTIDAD_RECIBIDA * (CASE WHEN OCD_AFC_FactorConversion > 0 THEN OCD_AFC_FactorConversion  ELSE 1 END) AS CANTIDAD, OCFR_PrecioUnitario AS COSTO,
              (OCFR_PrecioUnitario * (1-(OCFR_PorcentajeDescuento/100))) /
              (CASE WHEN OCD_AFC_FactorConversion > 0 THEN OCD_AFC_FactorConversion  ELSE 1 END) AS COSTO_OC,
             (CANTIDAD_RECIBIDA * (OCFR_PrecioUnitario * (1-(OCFR_PorcentajeDescuento/100)))) /
              (CASE WHEN OCD_AFC_FactorConversion > 0 THEN OCD_AFC_FactorConversion  ELSE 1 END) AS IMPORTE,
                
                (Select MON_Nombre from Monedas Where MON_MonedaId = OC_MON_MonedaId) AS MONEDA, EMP_CodigoEmpleado AS C_EMPL, RTRIM(EMP_Nombre)+' '+RTRIM(EMP_PrimerApellido) AS NOM_EMPL From OrdenesCompra inner join OrdenesCompraDetalle on OC_OrdenCompraId = OCD_OC_OrdenCompraId left JOIN Articulos ON OCD_ART_ArticuloId = ART_ArticuloId inner join OrdenesCompraFechasRequeridas on OC_OrdenCompraId = OCFR_OC_OrdenCompraId AND OCD_PartidaId = OCFR_OCD_PartidaId left join (SELECT SUM(OCRC_CantidadRecibo) AS CANTIDAD_RECIBIDA, OCRC_OC_OrdenCompraId, OCRC_OCFR_FechaRequeridaId, OCRC_EMP_ModificadoPorId, OCRC_FechaRecibo, OCRC_Comentarios, OCRC_PrecioOrdenCompraAlRecibir FROM OrdenesCompraRecibos GROUP BY OCRC_OC_OrdenCompraId, OCRC_OCFR_FechaRequeridaId, OCRC_FechaRecibo, OCRC_Comentarios, OCRC_PrecioOrdenCompraAlRecibir, OCRC_EMP_ModificadoPorId ) AS OCRecibos ON OC_OrdenCompraId = OCRecibos.OCRC_OC_OrdenCompraId AND OCFR_FechaRequeridaId = OCRecibos.OCRC_OCFR_FechaRequeridaId inner join Proveedores on OC_PRO_ProveedorId = PRO_ProveedorId left join Proyectos on OC_EV_ProyectoId = PRY_ProyectoId inner join Empleados on OCRC_EMP_ModificadoPorId = EMP_EmpleadoId where OC_Borrado = 0
            and Cast(OCRC_FechaRecibo As Date)  BETWEEN '" . date('Y-m-d', strtotime($request->get('fi'))) . ' 00:00' . "' and '" . date('Y-m-d', strtotime($request->get('ff'))) . ' 23:59:59' . "'
            "); 
        } else {
            $data = DB::connection($sociedad)->select( "
